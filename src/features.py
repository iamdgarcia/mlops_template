...
class FeatureEngineer:
    ...
    def create_temporal_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "timestamp" in df_temp.columns:
            ...
            df_temp["is_late_night"] = (
                (df_temp["hour_of_day"] >= 23) | 
                (df_temp["hour_of_day"] <= 6)
            ).astype(int)
            
            logger.info("Created temporal features: hour_of_day, day_of_week, is_weekend, time_category, is_business_hours, is_late_night")
    ...
    def create_amount_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "amount" in df_amount.columns:
            ...
            logger.info("Created amount features: amount_log, amount_category, round amount indicators")
    ...
    def create_user_behavior_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not existing_user_features:
            ...
            user_stats = df_user.groupby("user_id").agg({
        ...
        if "user_std_amount" not in df_user.columns:
            df_user["user_std_amount"] = df_user.groupby("user_id")["amount"].transform("std").fillna(0)
        ...
        df_user["amount_zscore"] = (
            (df_user["amount"] - df_user["user_avg_amount"]) / 
        ...
        logger.info("Created user behavior features: transaction counts, amount statistics, z-scores, ratios")
    ...
    def create_frequency_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_freq["time_since_last_transaction"] = (
            df_freq.groupby("user_id")["timestamp"]
            .diff()
            .dt.total_seconds()
            .fillna(0)
        )
        ...
        df_freq["transactions_per_day"] = df_freq["user_transaction_count"] / 30.0  # Assuming 30-day window
        ...
        df_freq["is_very_quick_transaction"] = (df_freq["time_since_last_transaction"] <= 300).astype(int)
    ...
    def _encode_categorical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("Encoding %d categorical features using categorical codes", len(categorical_cols))
    ...
    def get_feature_names(self, df: pd.DataFrame) -> List[str]:
        ...
        exclude_columns = [
            ...
            "user_frequent_location", "user_frequent_device", "user_frequent_merchant"
        ]
