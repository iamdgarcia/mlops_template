...
class ModelTrainer:
    ...
    def prepare_model_configs(self) -> Dict[str, Any]:
        ...
        if self.models_config.get("logistic_regression", {}).get("enabled", True):
            model_configs["logistic_regression"] = {
                ...
                "param_grid": self.models_config.get("logistic_regression", {}).get("hyperparameters", {
                    "C": [0.1, 1.0, 10.0],
        ...
        if XGBOOST_AVAILABLE and self.models_config.get("xgboost", {}).get("enabled", True):
            model_configs["xgboost"] = {
                ...
                "param_grid": self.models_config.get("xgboost", {}).get("hyperparameters", {
            ...
            rf_param_grid = rf_cfg.get("hyperparameters", {
                "n_estimators": [100],
                "max_depth": [5, None],
                "min_samples_split": [2],
                "min_samples_leaf": [1],
    ...
    def train_model(
        ...
        with mlflow.start_run(run_name=f"{model_name}_training") as run:
            ...
            if use_grid_search and config.get("param_grid"):
                ...
                best_params_to_log = {k: v for k, v in model.best_params_.items() if k not in non_tuned_params}
    ...
    def _register_best_model(self, model_name: str, run_id: Optional[str]) -> None:
        ...
        try:
            ...
            logger.info(f"Registered {model_name} as {registry_model_name} in MLflow Model Registry")
