...
class DataProcessor:
    ...
    def split_data(
        ...
        if validation_size and validation_size > 0:
            ...
            stratify_train_val = train_val_df[stratify_column] if stratify_column in train_val_df.columns else None
    ...
    def _get_feature_columns(self, df: pd.DataFrame, target_column: str) -> List[str]:
        ...
        exclude_columns = {
            "transaction_id", "user_id", "timestamp", target_column
        }
        
        # Add any additional columns to exclude from config
        exclude_columns.update(
            self.features_config.get("exclude_columns", [])
        )
...
def process_data(config: Dict[str, Any], input_data: Optional[pd.DataFrame] = None) -> Dict[str, pd.DataFrame]:
    """
